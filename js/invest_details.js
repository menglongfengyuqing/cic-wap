...
function getProjectInfoWap(userName) {
	$.ajax({
		...
		success: function(json) {
			console.log("出借： JSON = " +JSON.stringify(json))
			// 查找成功.
			if(json.state == 0) {
				var data = json.data;
				var creditUrl = data.creditUrl;
				...
				$("#projectspan").html(data.span + "天"); // 借款期限
...
//					}
				...
				//注册资本
				console.log("degug = " + data.ztmgLoanBasicInfoEntity)
				if(data.ztmgLoanBasicInfoEntity == null || (data.ztmgLoanBasicInfoEntity.registeredCapital == "" || data.ztmgLoanBasicInfoEntity.registeredCapital  == null)) {
					$("#borrowerRegisterAmount").html("---");
				} else {
					$("#borrowerRegisterAmount").html(data.ztmgLoanBasicInfoEntity.registeredCapital + "元");
				}
				//注册地址 
				if(data.ztmgLoanBasicInfoEntity == null || (data.ztmgLoanBasicInfoEntity.registeredAddress== "" || data.ztmgLoanBasicInfoEntity.registeredAddress == null)) {
					$("#address").html("---");
				} else {
				...
				//成立时间
				if(data.ztmgLoanBasicInfoEntity == null || (data.ztmgLoanBasicInfoEntity.setUpTime == "" || data.ztmgLoanBasicInfoEntity.setUpTime == null)) {
				...
				//法定代表人
		       if(data.ztmgLoanBasicInfoEntity == null || (data.ztmgLoanBasicInfoEntity.operName == "" || data.ztmgLoanBasicInfoEntity.operName == null)) {
				...
				if(data.proState == 3) {
					$("#invest_btn").addClass("once").html("即将上线");
					$(".repaymentPlan").remove();
					$("#interestDate").html("满标当日计息"); //起息时间
				} else if(data.proState == 4 && checkDate(new Date().format("yyyyMMddhhmmss"), data.loandate)) {
					$("#invest_btn").addClass("default").html("已过期");
					$(".repaymentPlan").remove();
					$("#interestDate").html("满标当日计息"); //起息时间
				} else if(data.proState == 4) {
					$("#invest_btn").html("立即加入").addClass("cur invest_btn_ok");
					$("#interestDate").html("满标当日计息"); //起息时间
					$(".repaymentPlan").remove();
					$(".invest_btn_ok").click(function() {
						$.ajax({
							url: ctxpath + "/user/getUserInfo",
							type: "post",
							dataType: "json",
							...
							success: function(result) {
								if(result.state == '0') {
									window.location.href = "invest_details_confirm.html?projectId=" + projectId;
								} else {
				...
				} else if(data.proState == "5") {
					$("#interestDate").html(data.loandate); //起息时间
					$("#invest_btn").addClass("default");
...
function projectInfo(data, rate, interestRateIncrease, creditUrl) {
	...
	$("#projectAmount").html(toFixed2(data.amount) + "元"); //借款金额
	$("#projectspan2").html(data.span); // 借款期限

	if(interestRateIncrease == 0) {
		$("#projectstate2").html(rate + "%"); //年化利率
		...
		var rateBasePI = parseInt(rateBase);
		var rateBasePF = rateBase.toString().split(".")[1];
		$("#projectstate2").html(rateBasePI + "." + rateBasePF + "%+" + interestRateIncrease + "%"); // 年化利率
	}

	$("#purpose").html(data.purpose); //借款用途
